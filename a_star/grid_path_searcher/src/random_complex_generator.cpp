...
void RandomMapGenerate() {
  ...
  for (int i = 0; i < _cir_num; i++) {
    ...
    if (sqrt(pow(x0 - _init_x, 2) + pow(y0 - _init_y, 2)) < 2.0) continue;
    ...
    for (auto pt : circle_set) {
      ...
      if (pt_random.z >= 0.0) cloudMap.points.push_back(pt_random);
    }
  }
  ...
  for (int i = 0; i < _obs_num; i++) {
    ...
    if (sqrt(pow(x - _init_x, 2) + pow(y - _init_y, 2)) < 0.8) continue;
    ...
    if (is_kdtree_empty == false) {
      if (kdtreeMap.nearestKSearch(searchPoint, 1, pointIdxSearch,
        ...
        if (sqrt(pointSquaredDistance[0]) < 1.0) continue;
      }
    }
